...
def remove_shift_(user, user_file, entry):
	...
	try:
		...
		cmd = "crontab -u %s %s" % (user, user_file)
...
#------------------------------------------------------------------------------------------------
logger.debug("Entering the job runner", extra=d)
...
if shift_time:
	...
	logger.info('The job is no shifted, provied %d argument' %len(sys.argv), extra=d)
...
if (history_list and history_list[0] and history_list[1]):
	...
	if history_id and storage.is_ready()[0] and  storage.connect():
		try:
			out = proc.stdout.read()
			err = proc.stderr.read()
			logger.debug("trying to save ending state out : %s, err : %s" %(out, err), extra=d)
